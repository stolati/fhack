...
"""
fspyemf.py is a modified version of pyemf.py. We use it to parse .spl files
that are uploaded from clients. This file offers lot of functionality that
we don't need, but functionality is retained in order to minimize changes from
original pyemf.py code. Functionality has also been added, in order to meet our needs
The main changes from pyemf.py are
1. Ability to parse .spl files instead of pure emf files. .spl files are emf files
   with extra records at beginning and end.
2. Ability to parse records of type _SMALLTEXTOUT
3. Hardcoded hacks for non-conforming emf files. Even though emf is a standard format
   many time the generated emf does not conform to standards and has inconsistencies. This is
   well known issue with emf. We have to workaround those issues by using various hacks.
4. We are mostly interested in extracting text output from emf. Thus we've added an attribute
   which collects the strings encountered in emf.
5. flake8/pep8 changes.
"""
...
class EMFParsingError(Exception):
    pass


class EMFError_WrongSizeInRecordHeader(EMFParsingError):
    pass
...
class _DC:
    ...
    http://msdn.microsoft.com/library/default.asp?url=/library/en-us/gdi/cordspac_3qsz.asp
    ...
    def removeObject(self, handle):
        ...
        # print "removing handle %d (%s)" %
        # (handle,self.objects[handle].__class__.__name__.lstrip('_'))
...
class _EMR_UNKNOWN(object):  # extend from new-style class, or __getattr__ doesn't work
    ...
    def __init__(self):
        ...
        # a different typedef
        if self.__class__.format is None or self.__class__.emr_typedef != self.format.typedef:
            if self.verbose:
    ...
    def unserialize(self, fh, itype=- 1, nsize=- 1):
        ...
        if itype > 0:
            ...
            data = fh.read(8)
            (self.iType, self.nSize) = struct.unpack("<ii", data)
        if self.nSize > 8:
            self.datasize = self.nSize - 8
            self.data = fh.read(self.datasize)

            """
            EMF parsing is notoriously difficult. Furthermore, many EMF's dont conform to standard.
            Thus many times we have to do hacks. This is one such hack for Sahil (a store in
            chicago).
            In some cases the size of record indicated is short by 78 bytes. Those extra bytes
            are all 0xFDFDFDFD. We will just check whether first 4 bytes are 0xFDFDFDFD and
            if so, we assume the remaining are also 0xFDFDFDFD.
            """
            if self.__class__.__name__ == "_STRETCHDIBITS":
                temp = fh.read(4)
                if temp == "\xfd\xfd\xfd\xfd":
                    # 4 bytes added earlier. Now add another 74
                    fh.read(74)
                else:
                    # 4 bytes added earlier. Undo it.
            ...
            if self.format.structsize > 0:
                ...
                self.values = list(
                    struct.unpack(self.format.fmt, self.data[0:self.format.structsize]))
            if self.datasize > self.format.structsize:
                return self.unserializeExtra(self.data[self.format.structsize:])
    ...
    def resize(self):
        ...
        if self.verbose and before != self.nSize:
            ...
            print(self)
        if self.nSize % 4 != 0:
            print("size error.  Must be divisible by 4", end=" ")
            print(self)
            raise TypeError
    ...
    def str_color(self, val):
        return "red=0x%02x green=0x%02x blue=0x%02x" % ((val & 0xff), ((val & 0xff00) >> 8),
                ((val & 0xff0000) >> 16))
    ...
    def __str__(self):
        ...
        return "**%s: iType=%s nSize=%s  struct='%s' size=%d\n%s%s" % (
            self.__class__.__name__.lstrip('_'), self.iType, self.nSize, self.format.fmt,
            self.format.structsize, details, ret)


class _EMRI_UNKNOWN(_EMR_UNKNOWN):
    pass
...
class _EMRI:
    ...
    class _EMRI_METAFILE_DATA(_EMRI_UNKNOWN):
        emri_id = 0x0000000C
...
for name in dir(_EMRI):
    # print name
    cls = getattr(_EMRI, name, None)
    if cls and callable(cls) and issubclass(cls, _EMRI_UNKNOWN):
        # print "subclass! id=%d %s" % (cls.emr_id,str(cls))
        _emrimap[cls.emri_id] = cls
...
class _EMR:
    ...
    class _HEADER(_EMR_UNKNOWN):
        ...
        def __init__(self, description=''):
            ...
            if len(description) > 0:
                self.description = u'pyemf ' + \
                    __version__.decode('utf-8') + u'\0' + description + u'\0\0'
        ...
        def str_extra(self):
            txt = BytesIO()
            txt.write(("\tunicode string: %s\n" % str(self.description)).encode('utf8'))
    ...
    class _POLYBEZIER(_EMR_UNKNOWN):
        ...
        def __init__(self, points=None, bounds=(0, 0, 0, 0)):
    ...
    class _POLYPOLYLINE(_EMR_UNKNOWN):
        ...
        def __init__(self, points=None, polycounts=None, bounds=(0, 0, 0, 0)):
            points = [] if points is None else points
            polycounts = [] if polycounts is None else polycounts
        ...
        def unserializeExtra(self, data):
            ...
            # print "aPolyCounts start=%d size=%d: %s" %
            # (start,len(self.aPolyCounts),str(self.aPolyCounts))
...
# define EMR_SETDIBITSTODEVICE  80
    ...
    class _EXTCREATEFONTINDIRECTW(_EMR_UNKNOWN):
        ...
        emr_typedef = [
            ...
            ('B', 'lfQuality'),
            ('B', 'lfPitchAndFamily'),
            ('64s', 'lfFaceName',),  # really a 32 char unicode string. i.e. 64 bytes
            # LOGFONT object ends here

            #  LOGFONT + fullname + style are found in both LOGFONTEX and LOGFONTPANOSE
            ('128s', 'elfFullName', '\0' * 128),  # really 64 char unicode str i.e. 128 bytes
            ('64s', 'elfStyle', '\0' * 64),  # really 32 char unicode str i.e. 64 bytes
...
# define EMR_STARTDOC      107
    class _SMALLTEXTOUT(_EXTTEXTOUTA):
        emr_id = 108
        emr_typedef = [
            ('i', 'rclBounds_left', 0),
            ('i', 'rclBounds_top', 0),
...
@group Drawing Parameters: GetStockObject, SelectObject, DeleteObject, CreatePen,
CreateSolidBrush, CreateHatchBrush, SetBkColor, SetBkMode, SetPolyFillMode
@group Drawing Primitives: SetPixel, Polyline, PolyPolyline, Polygon, PolyPolygon, Rectangle,
...
@group **Experimental** -- Viewport Manipulation: SetMapMode, SetViewportOrgEx, GetViewportOrgEx,
SetWindowOrgEx, GetWindowOrgEx, SetViewportExtEx, ScaleViewportExtEx, GetViewportExtEx,
SetWindowExtEx, ScaleWindowExtEx, GetWindowExtEx
...
"""
    ...
    def parse_file(self, filename):
        with open(filename, "rb") as fh:
            return self._parse(fh, os.stat(filename).st_size)

    def parse_str(self, emfstr):
        fh = StringIO.StringIO(emfstr)
        return self._parse(fh, len(emfstr))

    def parse_fhandle(self, fh, size=None):
        """
        fh is file handle. It could either be created by calling open on a file,
        or by using StringIO.StringIO()
        """
        if size is None:
            current = fh.tell()
            fh.seek(0, 2)  # offset 0 from end
            size = fh.tell() - current
            fh.seek(0, 0)  # offset 0 from begin
        return self._parse(fh, size)

    def _parse(self, fh, fsize):
        self.records = []
        self._unserialize_splheader(fh, fsize)

        # _unserialize parses all records in emf. But we only need TEXTOUT
        # records. So we've modified _unserialize to only capture strings
        # from TEXTOUT
        # EMF can contain unicode strings. In EMF world, a normal english unicode
        # character is the ascii character followed by \x00. But Python represents
        # unicode differently. Let's convert the unicode to normal ascii by
        # removing "\x00". Note that EMF may contain both ascii and unicode strings.
        # After removing \x00, all strings will look like ascii
        receipt_text = self._unserialize(fh, fsize).replace("\x00", "")

        self.scaleheader = False
        # get DC from header record
        self.dc.getBounds(self.records[0])
        self._unserialize_splfooter(fh)
        return receipt_text

    def _unserialize_splheader(self, fh, fsize):
        data = fh.read(8)
        (signature, nSize) = struct.unpack("<ii", data)
        if nSize > fsize - 8:
            raise EMFError_WrongSizeInRecordHeader("Incorrect nSize encountered while parsing "
                    "first 8 bytes of spl file. Can't continue parsing")

        fh.read(nSize)
        # Now we have read two headers specific to spl format.
        # type 0x1 = EMRI_METAFILE and type 0xc = EMRI_METAFILE_DATA
        # fh now points to first byte of actual emf

    def _unserialize_splfooter(self, fh):
        pass

    def _unserialize(self, fh, fsize):
        tm = TextMap()
        receipt_text = ""
        try:
            count = 1
            while count > 0:
                # recbegin_pos = record being position.
                recbegin_pos = fh.tell()
                data = fh.read(8)  # Read Header which is 8 bytes
                count = len(data)
                if count > 0:
                    (iType, nSize) = struct.unpack("<ii", data)

                    if self.verbose:
                        print("EMF:  offset=%d(0x%x), iType=%d(0x%x) nSize=%d(0x%x)" % (
                            recbegin_pos, recbegin_pos, iType, iType, nSize, nSize))

                    if nSize + recbegin_pos > fsize and fsize - fh.tell() > 8:
                        """
                        The first if statement says that if the size of the record is greater
                        than the number of remaining characters to be read, then its an error.
                        It indicates EMF is malformed.

                        The second check makes an exception to the firt check.
                        In one specific case, even though the EMF is malformed, we are ok
                        working around the error. In a receipt from Smoothie Factory, a record
                        says that its size is 0x24 bytes but actually it is only 0x20 bytes. So
                        the parser reads next record at offset 0x24 bytes instead of 0x20. It
                        thus ends up reading the new record header from wrong location. Thus
                        header read is wrong, and the record size indicated by that header is
                        wrong.
                        Luckily in this case, the record read wrong is the last record, and
                        the last record is never a TextOut record. It is usually EOF record.
                        So we can ignore this error. To put it in a general way, we can say
                        that throw record size error only if the number of remaining
                        bytes in EMF are more than size of EMF record header(which is 8).
                        """
                        raise EMFError_WrongSizeInRecordHeader("Incorrect nSize encountered "
                            "while parsing emf record. Can't continue parsing")
                    ...
                    text = e.unserialize(fh, iType, nSize)
                    ...
                    if self.verbose:
                        print("Just unserialized: ", end=" ")
                        # (hsingh)This statement causes exception sometimes. But it is only
                        # for debugging so it is fine.
                        try:
                            print(e)
                        except:
                            pass

                    if e.__class__.__name__ == "_EOF":
                        break
...
@rtype: Boolean
    ...
    def _useShort(self, bounds):
        ...
        SHRT_MIN = - 32768
        SHRT_MAX = 32767
        if (bounds[0] >= SHRT_MIN and bounds[1] >= SHRT_MIN and bounds[2] <= SHRT_MAX and
                bounds[3] <= SHRT_MAX):
            return True
        return False
...
@type color: int
        ...
        """
        return self._appendHandle(_EMR._CREATEBRUSHINDIRECT(hatch=hatch,
...
Set the window mapping mode.  This is the mapping between pixels in page space to pixels in
device space.  Page space is the coordinate system that is used for all the drawing
commands -- it is how pixels are identified and figures are placed in the metafile.
They are integer units.

Device space is the coordinate system of the final output, measured in physical dimensions such
as mm, inches, or twips.  It is this coordinate system that provides the scaling that makes
metafiles into a scalable graphics format.
...
@type y: int
        ...
        self.dc.viewport_ext_x = x
        self.dc.viewport_ext_y = y
        return old
...
modified in one of three ways, set by the mode parameter:
 ...
 - MWT_LEFTMULTIPLY: multiply the matrix represented by these parameters by the current world
   transform to get the new transform.
 - MWT_RIGHTMULTIPLY: multiply the current world tranform by the matrix represented here to get
   the new transform.
...
@type points: tuple
        ...
        if len(points) == 4:
            if (points[0][0] == points[1][0] and points[2][0] == points[3][0] and
                    points[0][1] == points[3][1] and points[1][1] == points[2][1]):
                if self.verbose:
                    print("converting to rectangle, option 1:")
                return self.Rectangle(points[0][0], points[0][1], points[2][0], points[2][1])

            elif (points[0][1] == points[1][1] and points[2][1] == points[3][1] and
                  points[0][0] == points[3][0] and points[1][0] == points[2][0]):
                if self.verbose:
...
Close a currently open path, which connects the current position to the starting position of
a figure.  Usually the starting position is the most recent call to L{MoveTo} after L{BeginPath}.
...
@param mode: one of the following values that specifies how to modify the clipping path
 ...
 - RGN_XOR: the new clipping path becomes the union of the old path and the current path minus
   the intersection of the old and current path
 - RGN_DIFF: the new clipping path becomes the old path where any overlapping region of the current
   path is removed
...
@type color: int
    ...
    def CreateFont(self, height, width=0, escapement=0, orientation=0, weight=FW_NORMAL,
            italic=0, underline=0, strike_out=0, charset=ANSI_CHARSET,
            out_precision=OUT_DEFAULT_PRECIS, clip_precision=CLIP_DEFAULT_PRECIS,
            quality=DEFAULT_QUALITY, pitch_family='DEFAULT_PITCH|FF_DONTCARE',
...
@param escapement: angle, in degrees*10, of rendered string rotation.  Note that escapement and
orientation must be the same.
@param orientation: angle, in degrees*10, of rendered string rotation.  Note that escapement
and orientation must be the same.
...
values:
 ...
 - ANTIALIASED_QUALITY
@param pitch_family: the pitch and family of the font face if the named font can't be found.
Combine the pitch and style using a binary or.
 - Pitch:
   - DEFAULT_PITCH
   - FIXED_PITCH
   - VARIABLE_PITCH
...
@type name: string
        ...
        """
        return self._appendHandle(_EMR._EXTCREATEFONTINDIRECTW(height, width, escapement,
            orientation, weight, italic, underline, strike_out, charset, out_precision,
